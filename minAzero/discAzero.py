...
for k in range(nU):
    ...
    Lcouple.append('d'+ h[k+3+nY+nY+nP]) # this needs a non-degenerate name
for k in range(nM):
...
for i in range(nU):
  sTemp2 = "Sk[%d]" % (2*i+1)
  sTemp1 = sTemp1.replace(Lcouple[2*i+1],sTemp2)
  sTemp2a = "Sk2[%d]" % (2*i+1)
  sTemp1a = sTemp1a.replace(Lcouple[2*i+1],sTemp2a)
for i in range(nU):
  sTemp2 = "Sk[%d]" % 2*i
  sTemp1 = sTemp1.replace(Lcouple[2*i],sTemp2)
  sTemp2a = "Sk2[%d]" % i
  sTemp1a = sTemp1a.replace(Lcouple[2*i],sTemp2a)
...
for k in range(Fdim):
  ...
  for i in range(nU):
    sTemp2 = "Sk[%d]" % (2*i+1)
    sTemp1 = sTemp1.replace(Lcouple[2*i+1],sTemp2)
    sTemp2a = "Skp1[%d]" % (2*i+1)
    sTemp1a = sTemp1a.replace(Lcouple[2*i+1],sTemp2a)
    sTemp2b = "Sk2[%d]" % (2*i+1)
    sTemp1b = sTemp1b.replace(Lcouple[2*i+1],sTemp2b)
  for i in range(nU):
    sTemp2 = "Sk[%d]" % 2*i
    sTemp1 = sTemp1.replace(Lcouple[2*i],sTemp2)
    sTemp2a = "Skp1[%d]" % 2*i
    sTemp1a = sTemp1a.replace(Lcouple[2*i],sTemp2a)
    sTemp2b = "Sk2[%d]" % 2*i
    sTemp1b = sTemp1b.replace(Lcouple[2*i],sTemp2b)
  for i in range(nM):
    sTemp2 = "Sd[%d]" % i
    sTemp1 = sTemp1.replace(Ldata[i],sTemp2)
    sTemp2a = "Sdp1[%d]" % i
